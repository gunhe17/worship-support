...
import { addSongListItem, removeSongListItem, reorderSongListItem } from "../actions";
import { Alert, FormSelect } from "@/components";
import type { FormState } from "@/lib/form-types";

const initialState: FormState = { error: null, success: null };

export function SongListItemsPanel({
  parentBlockId,
  songs,
  items,
}: {
  ...
  const [addState, addAction] = useActionState(addSongListItem, initialState);
  const [removeState, removeAction] = useActionState(removeSongListItem, initialState);
  const [reorderState, reorderAction] = useActionState(reorderSongListItem, initialState);
        ...
        <form action={addAction} className="flex flex-col gap-3 sm:flex-row sm:items-end">
          ...
          <div className="w-full">
            <FormSelect
              ...
              id="childBlockId"
              name="childBlockId"
              options={songs.map((song) => ({ value: song.id, label: song.name }))}
            />
          </div>
          <button
            type="submit"
          ...
          <table className="min-w-full divide-y divide-gray-700">
            <thead>
              <tr className="text-left text-sm font-semibold text-gray-100">
                ...
                <th className="px-6 py-4">Song 블록</th>
